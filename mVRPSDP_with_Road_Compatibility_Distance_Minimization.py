# Print time in file name and what solvers may be available
print()
# This is Minimizing Distance
...
# Both ways are possible. We may create a DICTIONARY or LIST of the Distance Matrix for C[i,j,k]
#But since Dictionary can have Keys as Strings, we use the DICTIONARY
C={} #This is the COST/DISTANCE Matrix
for k in Vehicle_Types:
    temp = pd.read_excel("Input Data.xlsx",str(k))
    for i,row in temp.iterrows():
        if row["Origin_Node"] in Depot_and_Relief_Centres:
            if row["Destination_Node"] in Depot_and_Relief_Centres:
                key=(int(row["Origin_Node"]),int(row["Destination_Node"]),int(k))
...
prob+=p.lpSum(x[i,j,k]*C[i,j,k] for k in Vehicle_Types for i in Depot_and_Relief_Centres for j in Depot_and_Relief_Centres)
...
#Ensuring the PickUp constraints are satisfied (Point 3 d)
for i in Relief_Centres:
    prob+=p.lpSum(y[i,j,k]-y[j,i,k] for j in Depot_and_Relief_Centres for k in Vehicle_Types)==pp[i]

#Ensuring the Delivery constraints are satisfied (Point 3 e)
for i in Relief_Centres:
...
#Ensuring initial PickUp from Nodes is 0 (Point 3 h i)
for k in Vehicle_Types:
    for j in Relief_Centres:
        prob+=y[0,j,k]==0

#Ensuring final Delivery to Nodes is 0 (Point 3 h iI)
for k in Vehicle_Types:
    for i in Relief_Centres:
        prob+=z[i,0,k]==0
